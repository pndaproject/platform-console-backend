...
frisby.create('Get packages')
  .get('http://localhost:3123/api/dm/packages')
  .expectStatus(200)
  .expectHeaderContains('Content-Type', 'application/json')
  .expectJSONTypes('packages.*', {
    name: String,
    available_versions: Array,
    deployed_versions: Array
  })
  .toss();
 
frisby.create('Get package')
  .get('http://localhost:3123/packages/psm') // how to get key to test one value?
  .expectStatus(200)
  .expectHeaderContains('Content-Type', 'application/json')
  .expectJSONTypes({
    status: String,
    information: function(val) { expect(val).toBeTypeOrNull(String); },
    version: String,
    name: String,
    defaults: function(val) { expect(val).toBeTypeOrNull(String); },
  })
  .toss();
