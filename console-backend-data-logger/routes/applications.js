...
* Purpose:     Route for POST /applications API
*
* Author:      PNDA Team
* Created:     2016/02/01
  ...
  if (req.body) {
    async.each(req.body.data, function(item, callback) {
      if (item.id != null && item.id !== "") {
        ...
        callback('Error - missing required data in application body...', item);
      }
    }, function(err) {
      if (err === null) {
        logger.debug("Logged new application data okay");
        res.sendStatus(200);
      } else {
        logger.error("Failed to log new application data: ", err);
        res.sendStatus(500);
      }
    });
  } else {
    logger.error("Missing required application params");
    res.sendStatus(400);
  }
});
